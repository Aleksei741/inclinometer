...
class Truck3AxelWindow(CarFrame):
    def __init__(self, master, vehicle_data: VehicleAlignmentData):
        ...
        self.creatTableFixedWheelParams(main_frame, 2, "left", axle_twist=False).grid(row=6, column=0, padx=5, pady=(20,0), sticky='n')         

        # ----------- Таблицы параметров правого фиксированного колеса 2 ----------
        self.creatTableFixedWheelParams(main_frame, 2, "right", axel_shift=False).grid(row=6, column=2, padx=5, pady=(20,0), sticky='n')
        ...
        # Задняя ось 2
        self.creatTableTotalToe(main_frame, 2).grid(row=6, column=1, padx=1, pady=(10,0), sticky='n')