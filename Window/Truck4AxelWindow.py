...
class Truck4AxelWindow(CarFrame):
    def __init__(self, master, vehicle_data: VehicleAlignmentData):
        ...
        self.table_l1 = self.creatTableSteerableWheelParams(main_frame, 1, "left")
        self.table_l1.grid(row=2, column=0, padx=5)

        # ----------- Таблицы параметров правого подруливающего колеса 2 ---------
        self.table_r1 = self.creatTableSteerableWheelParams(main_frame, 1, "right")
        self.table_r1.grid(row=2, column=2, padx=5)
        ...
        self.table_l3 = self.creatTableFixedWheelParams(main_frame, 3, "left")
        self.table_l3.grid(row=5, column=0, padx=5) 
        
        # ----------- Таблицы параметров правого фиксированного колеса 2 ----------
        self.table_r3 = self.creatTableFixedWheelParams(main_frame, 3, "right")
        ...
        # Задняя ось 2
        self.creatTableTotalToe(main_frame, 1).grid(row=2, column=1, padx=1)
        # Задняя ось 1
        self.creatTableTotalToe(main_frame, 2).grid(row=4, column=1, padx=1)
        # Задняя ось 2
        self.creatTableTotalToe(main_frame, 3).grid(row=5, column=1, padx=1)