...
class Trailer3AxelWindow(CarFrame):
    def __init__(self, master, vehicle_data: VehicleAlignmentData):
        ...
        # ----------- Общее схождение -----------
        tk.Label(main_frame, text="Общее схождение оси:", font=("Arial", 10, "bold")).grid(row=0, column=1, pady=0)

        # ----------- Картинка шасси -----------
        self.img = tk.PhotoImage(file="img/TRAILER_3_AXLE.png")
        tk.Label(main_frame, image=self.img).grid(row=1, column=1, rowspan=5, columnspan=1)
        ...
        self.creatTableFixedWheelParams(main_frame, 1, "left").grid(row=4, column=0, padx=5) 
        
        # ----------- Таблицы параметров правого фиксированного колеса 2 ----------
        self.creatTableFixedWheelParams(main_frame, 1, "right").grid(row=4, column=2, padx=5)

        # ----------- Таблицы параметров левого фиксированного колеса 3 ----------
        self.creatTableFixedWheelParams(main_frame, 1, "left").grid(row=5, column=0, padx=5) 
        
        # ----------- Таблицы параметров правого фиксированного колеса 3 ----------
        self.creatTableFixedWheelParams(main_frame, 1, "right").grid(row=5, column=2, padx=5)

        # ----------- Спинбоксы "Общее схождение" поверх картинки -----------
        # Задняя ось 1
        self.creatTableTotalToe(main_frame, 0).grid(row=3, column=1, padx=1)
        # Задняя ось 2
        self.creatTableTotalToe(main_frame, 1).grid(row=4, column=1, padx=1)
        # Задняя ось 3
        self.creatTableTotalToe(main_frame, 2).grid(row=5, column=1, padx=1)