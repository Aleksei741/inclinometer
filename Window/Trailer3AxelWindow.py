...
class Trailer3AxelWindow(CarFrame):
    def __init__(self, master, vehicle_data: VehicleAlignmentData):
        ...
        self.bg_label.grid(row=1, column=1, rowspan=7, columnspan=1)
        ...
        self.table_l3.grid(row=6, column=0, padx=5) 
        
        # ----------- Таблицы параметров правого фиксированного колеса 2 ----------
        self.table_r3 = self.creatTableFixedWheelParams(main_frame, 1, "right")
        self.table_r3.grid(row=6, column=2, padx=5)

        # ----------- Таблицы параметров левого фиксированного колеса 3 ----------
        self.table_l3 = self.creatTableFixedWheelParams(main_frame, 1, "left")
        self.table_l3.grid(row=7, column=0, padx=5) 
        
        # ----------- Таблицы параметров правого фиксированного колеса 3 ----------
        self.table_r3 = self.creatTableFixedWheelParams(main_frame, 1, "right")
        self.table_r3.grid(row=7, column=2, padx=5)

        # ----------- Спинбоксы "Общее схождение" поверх картинки -----------
        # Задняя ось 1
        self.creatTableTotalToe(main_frame, 0).grid(row=5, column=1, padx=1)
        # Задняя ось 2
        self.creatTableTotalToe(main_frame, 1).grid(row=6, column=1, padx=1)
        # Задняя ось 3
        self.creatTableTotalToe(main_frame, 2).grid(row=7, column=1, padx=1)